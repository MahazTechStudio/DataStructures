...
private:
	...
	void incrementBy(int x, binaryTreeNode<elemType>*& p);
};
...
template<class Type>
void bSearchTreeType<Type>::incrementBy(int x, binaryTreeNode<Type>*& p)
{
	if (p != NULL)
	{
		p->info += x;
		incrementBy(x, p->llink);
		incrementBy(x, p->rlink);
	}
}

template <class Type>
void bSearchTreeType<Type>::incrementByWrapper(int x)
{
	incrementBy(x, this->root);
}
